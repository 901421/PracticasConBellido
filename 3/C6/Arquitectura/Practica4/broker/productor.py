...
def main():
    ...
    """
    # Capturamos la IP y Puerto del broker pasados como argumentos.
    # Uso: python productor.py <ip> [puerto]
    broker_ip = sys.argv[1] if len(sys.argv) > 1 else "127.0.0.1"
    broker_port = int(sys.argv[2]) if len(sys.argv) > 2 else 5555
    
    # Instanciamos el cliente conectándolo a la IP y Puerto resueltos
